...
FixedCamera camera(glm::vec3(0.0f, 2.5, 0),glm::vec3(0, 0, 0),glm::vec3(0, 0, -1.0));
...
int main()
{
    ...
    //GLuint backgroundTex = GenerateBackgroundTexture();
...
//    TransferFunctionControlPoint(1.0f, 1.0f, .85f, 256)
    vector<TransferFunctionControlPoint> colorKnots = {
        TransferFunctionControlPoint(1.0f, 1.0f, 1.0f, 0),
        TransferFunctionControlPoint(1.0f, 1.0f, 1.0f, 80),
        TransferFunctionControlPoint(1.0f, 1.0f, 1.0f, 82),
    ...
    Shader firstPassShader("./volume2/firstPass.vert", "./volume2/firstPass.frag");
    ...
    GLuint ggtex = CreateTexInfo();
    GLuint ggtexpic = GenerateBackgroundTexture();
    ...
    glBindBuffer(GL_ARRAY_BUFFER, ggtex);
    ...
    GLuint renderTextureFront, renderTextureBack, renderTex;

    renderTextureFront = setUpEmptyTexture();

    renderTextureBack = setUpEmptyTexture();

    renderTex = setUpEmptyTexture();
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, renderTex, 0);

    // Set the list of render Targets.
    GLenum renderTargets[3] = { GL_COLOR_ATTACHMENT0 , GL_COLOR_ATTACHMENT1 ,GL_COLOR_ATTACHMENT2};
    glDrawBuffers(3, renderTargets); // "2" is the size of renderTargets
    ...
    //Second Pass Setup
    //build and compile our shader program
    Shader raycastDiffuseShader("./volume2/raycastDiffuse.vert", "./volume2/raycastDiffuse.frag");
    Shader raycastShader("./volume2/raycast.vert", "./volume2/raycast.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindTexture(GL_TEXTURE_2D, ggtexpic);//绑定纹理
        ...
        if (currentShaderIndex == 2)
        {
            ...
            glUniform1i(glGetUniformLocation(currentRaycastShader->ID, "backgroundTex"), 6);
        }
